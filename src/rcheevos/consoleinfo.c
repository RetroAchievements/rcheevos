...
static const rc_memory_region_t _rc_memory_regions_ms_dos[] = {
     ...
     * Then the remaining conventional memory which contains DOS and BIOS controlled
     * memory at $100000. The length of this can vary depending on the hardware
     * and DOS version (or emulated DOS shell).
     * These first two regions will only ever total to 640 KB but the regions map
     * to 1 MB each to make resulting memory addresses more readable.
     */
    { 0x00000000U, 0x000FFFFFU, 0x00000000U, RC_MEMORY_TYPE_SYSTEM_RAM, "Game Conventional Memory" },
    { 0x00100000U, 0x001FFFFFU, 0x00100000U, RC_MEMORY_TYPE_SYSTEM_RAM, "OS Conventional Memory" },
...
static const rc_memory_regions_t rc_memory_regions_ms_dos = { _rc_memory_regions_ms_dos, 3 };
