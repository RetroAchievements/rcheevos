...
static void rc_client_raise_server_error_event(rc_client_t* client,
    const char* api, uint32_t related_id, const char* error_message)
{
...
static void rc_client_award_achievement_callback(const rc_api_server_response_t* server_response, void* callback_data)
{
  ...
  if (error_message) {
    if (award_achievement_response.response.error_message && !rc_client_should_retry(server_response)) {
      ...
      rc_client_raise_server_error_event(ach_data->client, "award_achievement", ach_data->id, award_achievement_response.response.error_message);
    }
      ...
      if (!ach_data->scheduled_callback_data) {
        ...
        if (!ach_data->scheduled_callback_data) {
          ...
          rc_client_raise_server_error_event(ach_data->client, "award_achievement", ach_data->id, rc_error_str(RC_OUT_OF_MEMORY));
          return;
        }
...
static void rc_client_award_achievement(rc_client_t* client, rc_client_achievement_info_t* achievement)
{
  ...
  if (!callback_data) {
    ...
    rc_client_raise_server_error_event(client, "award_achievement", achievement->public_.id, rc_error_str(RC_OUT_OF_MEMORY));
    return;
  }
  callback_data->client = client;
...
static void rc_client_submit_leaderboard_entry_callback(const rc_api_server_response_t* server_response, void* callback_data)
{
  ...
  if (error_message) {
    if (submit_lboard_entry_response.response.error_message && !rc_client_should_retry(server_response)) {
      ...
      rc_client_raise_server_error_event(lboard_data->client, "submit_lboard_entry", lboard_data->id, submit_lboard_entry_response.response.error_message);
    }
      ...
      if (!lboard_data->scheduled_callback_data) {
        ...
        if (!lboard_data->scheduled_callback_data) {
          ...
          rc_client_raise_server_error_event(lboard_data->client, "submit_lboard_entry", lboard_data->id, rc_error_str(RC_OUT_OF_MEMORY));
          return;
        }
...
static void rc_client_submit_leaderboard_entry(rc_client_t* client, rc_client_leaderboard_info_t* leaderboard)
{
  ...
  if (!callback_data) {
    ...
    rc_client_raise_server_error_event(client, "submit_lboard_entry", leaderboard->public_.id, rc_error_str(RC_OUT_OF_MEMORY));
    return;
  }
  callback_data->client = client;
